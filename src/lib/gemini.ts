...
/**
 * Helper to compute seconds from time string e.g. "3 minutes" or "60s"
 */
function parseDurationToTotalSeconds(durationStr?: string): number {
  ...
  if (lower.includes('sec') || lower.includes('s')) {
    const num = parseFloat(lower.replace(/[^0-9.]/g, '')) || 60;
...
Create distinctive, fully realized character profiles with LOCKED VISUAL CONSISTENCY PROFILES for the following video project.

PROJECT SPECIFICATIONS:
- Video Idea: "${idea}"
- Concept Premise: "${concept?.premise || idea}"
- Creative Angle: "${concept?.coreAngle || 'Dynamic narrative'}"
- Visual Style: "${settings.visualStyle}"
- Tone: "${settings.tone}"
- Language: "${settings.language}"
- Narration Style: "${settings.narration || 'Voiceover / Host'}"
- Script Context: "${script?.completeScript?.slice(0, 800) || script?.narratorDialogue?.slice(0, 400) || 'Main storyline'}"

Generate 1 to 3 primary recurring characters/hosts (or key subject personas) appropriate for this video topic.
Each character MUST contain rich, locked visual reference attributes so image & video diffusion models can render them with 100% appearance continuity.
...
[
  {
    "id": "char-1",
    "name": "Character Name",
    "role": "Role (e.g. Lead Host / Investigator / Protagonist / Specialist)",
    "age": "Specific age or age bracket (e.g. 32 years old)",
    "gender": "Gender when relevant",
    "appearance": "Comprehensive overall visual description combining face, build, skin, and presence in ${settings.visualStyle}",
    "face": "Detailed facial features: bone structure, eye shape/color, eyebrows, nose, mouth, expressions",
    "hair": "Exact hairstyle, cut, length, volume, texture, and color",
    "skinOrVisualCharacteristics": "Skin tone, undertone, complexion texture, lighting highlights",
    "bodyOrBuild": "Height, body type, posture, silhouette",
    "clothing": "Exact signature clothing outfit, layers, fabrics, color palette, footwear",
    "accessories": "Signature props, glasses, jewelry, tech gadgets, or items always carried",
    "personality": "Key personality traits, charisma, demeanor, temperament",
    "personalityTraits": ["Trait 1", "Trait 2", "Trait 3"],
    "expressions": "Signature facial expressions, emotional demeanor on camera",
    "voice": "Voice characteristics, timbre, cadence, and accent in ${settings.language}",
    "speakingStyle": "Pacing, speech patterns, favorite phrasing, energy level",
    "characterPurpose": "Strategic role in driving story engagement and viewer connection",
    "visualPromptAnchor": "A concise, comma-separated diffusion prompt snippet locking their exact physical features, clothing, and style for 100% consistent AI generation (e.g. '32yo charismatic investigator, sharp jawline, messy dark brown taper fade, tailored obsidian trench coat with rolled sleeves, subtle glowing amber holographic lapel pin, cinematic studio rim lighting, 8k octane render ${settings.visualStyle}')"
  }
]
...
Respond ONLY with valid JSON (array of objects).`;
  ...
  const characters: CharacterProfile[] = list.map((c: any, index: number) => ({
    ...
    role: c.role || (index === 0 ? 'Lead Host' : 'Supporting Subject'),
    age: c.age || c.ageOrSpecies || 'Late 20s',
    ageOrSpecies: c.ageOrSpecies || c.age || 'Late 20s',
    gender: c.gender || 'Unspecified',
    appearance: c.appearance || c.visualAppearance || `${c.name || 'Character'} with a sharp, distinctive look rendered in ${settings.visualStyle}.`,
    visualAppearance: c.visualAppearance || c.appearance || `${c.name || 'Character'} in ${settings.visualStyle} style.`,
    face: c.face || 'Refined facial symmetry with expressive eyes.',
    hair: c.hair || 'Stylized modern hair matching visual palette.',
    skinOrVisualCharacteristics: c.skinOrVisualCharacteristics || c.skin || 'Even tone with warm cinematic rim highlights.',
    bodyOrBuild: c.bodyOrBuild || 'Athletic, upright posture with confident presence.',
    clothing: c.clothing || c.clothingOutfit || 'Signature contemporary tailored outfit.',
    clothingOutfit: c.clothingOutfit || c.clothing || 'Signature contemporary tailored outfit.',
    accessories: c.accessories || c.signatureItem || 'Minimalist signature accessory.',
    signatureItem: c.signatureItem || c.accessories || 'Minimalist signature accessory.',
    personality: c.personality || 'Engaging, observant, and articulate.',
    personalityTraits: Array.isArray(c.personalityTraits) ? c.personalityTraits : ['Charismatic', 'Insightful', 'Focused'],
    expressions: c.expressions || 'Intense focus shifting to welcoming curiosity.',
    voice: c.voice || c.voiceStyle || `Confident, resonant delivery in ${settings.language}.`,
    voiceStyle: c.voiceStyle || c.voice || `Confident delivery in ${settings.language}.`,
    speakingStyle: c.speakingStyle || 'Clear, punchy, articulate.',
    characterPurpose: c.characterPurpose || 'Anchor viewer attention and guide narrative journey.',
    visualPromptAnchor: c.visualPromptAnchor || `${c.name}, ${c.age || 'young adult'}, ${c.clothing || 'signature outfit'}, ${settings.visualStyle} aesthetic, cinematic lighting, 8k resolution`,
  }));
...
export async function generateScenesAI(
  ...
  const targetCount = Number(settings.targetScenesCount || settings.sceneCount) || 5;
  const totalSeconds = parseDurationToTotalSeconds(settings.targetDuration || settings.duration);
  const avgSecondsPerScene = Math.max(10, Math.round(totalSeconds / targetCount));

  const charactersContext = characters && characters.length > 0
    ? characters.map((c) => `- ${c.name} (${c.role}): Anchor Prompt: [${c.visualPromptAnchor || c.clothingOutfit || c.visualAppearance}]`).join('\n')
  ...
  const prompt = `You are a Hollywood Cinematographer and Senior AI Video Director (Runway, Sora, Luma Dream Machine, Midjourney v6).
...
- Target Duration: "${settings.targetDuration || settings.duration || '3 minutes'}" (Approx total ${totalSeconds} seconds)
...
Maintain strict continuity: character appearances, signature outfits, locations, and narrative progression must logically flow from scene to scene.
...
[
  {
    ...
    "location": "Specific location and environment setting",
    "timeOfDay": "Time of day (e.g. Dawn / Midnight / Studio)",
    "characters": ["Character Name 1"],
    "charactersPresent": ["Character Name 1"],
    "characterActions": "Explicit physical actions, blocking, gestures, facial reactions",
    "environment": "Detailed set dressing, background objects, depth, atmosphere in ${settings.visualStyle}",
    "visualDescription": "Cinematic composition, foreground/background layers, color grading",
    "dialogue": "Exact spoken line or narration in ${settings.language}",
    "dialogueVoiceover": "Exact spoken dialogue or voiceover in ${settings.language}",
    "narrator": "Narrator voiceover line",
    "camera": "Lens and framing (e.g. 35mm Low-Angle Hero Shot / 85mm Close-Up)",
    "cameraMovement": "Camera motion (e.g. Smooth forward dolly, rapid tilt up, slow orbital pan)",
    "cameraAngleMotion": "Lens + camera motion combined",
    "lighting": "Lighting setup and mood (e.g. High-contrast volumetric neon rim lighting, warm key light)",
    "lightingMood": "Lighting mood summary",
    "animation": "Motion dynamics, particle effects, speed ramp",
    "animationStyle": "Motion style description",
    "soundEffects": "Diegetic sound effects (e.g. High voltage hum, heavy footstep on marble)",
    "music": "Music cue and tempo (e.g. Dark rhythmic bass synth pulsing at 120 BPM)",
    "musicCue": "Music cue description",
    "transition": "Transition into next scene (e.g. Match cut on action / Whip pan / Hard cut)",
...
Ensure the total scene durations sum up to approximately ${totalSeconds} seconds.
Respond ONLY with valid JSON.`;
...
3. DEDICATED MODEL FORMATS: Provide customized, optimized video prompt strings for:
  ...
  "videoPrompts": [
    {
      ...
      "dialogue": "Exact spoken line or narration in ${settings.language}",
      "voiceAudio": "Vocal tone, cadence, timbre, and delivery pacing",
