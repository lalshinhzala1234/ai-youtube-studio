...
/**
 * Generate the ACTIVE story itself. This is intentionally the first AI stage so
 * every downstream asset is derived from the same story text instead of a
...
export async function generateStoryAI(
  ...
  const supplied = userStory?.trim();
  if (supplied) {
    return {
      storyMode: settings.storyMode === 'user_exact' ? 'user_exact' : 'user_refined',
      storySource: 'user_story',
      sourceText: supplied,
      fullStory: supplied,
      refinedStory: supplied,
      exactStory: settings.storyMode === 'user_exact' ? supplied : undefined,
      charactersInvolved: [],
      progression: [],
      progressionBeats: [],
    };
  ...
  const targetScenes = Number(settings.targetScenesCount || settings.sceneCount) || 6;
  const voiceMode = settings.voiceMode || settings.narration || 'Narrator + Character Dialogue';
  const noSpoken = String(voiceMode).toLowerCase().includes('no spoken');

  const prompt = `You are the STORY WRITER for a production pipeline. Create one completely original, self-contained story from the user's idea. The story you write becomes the SINGLE SOURCE OF TRUTH for every later module: characters, props, environments, scenes, script and video prompts.

ACTIVE USER IDEA: "${idea}"
VIDEO TYPE: "${settings.videoType || 'Kids'}"
AUDIENCE: "${settings.audience || 'General'}"
LANGUAGE: "${settings.language || 'English'}"
STYLE: "${settings.visualStyle || '3D Cartoon'}"
TONE: "${settings.tone || 'Fun'}"
TARGET DURATION: "${settings.targetDuration || settings.duration || '1 minute'}"
TARGET SCENES: ${targetScenes}
VOICE MODE: "${voiceMode}"
${settings.characterInstructions ? `CHARACTER INSTRUCTIONS: "${settings.characterInstructions}"` : ''}

HARD RULES:
1. Stay faithful to the user's idea. Do not substitute an unrelated adventure, forest, cave, sanctuary, compass, key, artifact, safari, alphabet story, or any previous/template story unless the idea actually requires it.
2. If the idea names characters, use those exact names and keep them consistent.
3. Explicitly establish each important character's age/category, appearance, clothing and role when appropriate, so downstream asset extraction does not have to invent them.
4. Explicitly establish important props and locations in the narrative when the idea implies them.
5. Never invent a generic "key narrative artifact" or generic location just to fill a slot.
6. ${noSpoken ? 'Do not write spoken dialogue; keep the story visual/narrative.' : `Write natural ${settings.language || 'English'} dialogue when characters speak.`}
7. The story must have a clear beginning, escalation, payoff and resolution.
8. Return only valid JSON.

JSON SHAPE:
{
  "fullStory": "Complete production-ready story in ${settings.language || 'English'} with clear paragraphs/scenes",
  "summary": "Short story summary",
  "premise": "One concise premise",
  "characterOverview": "List every distinct character and their key identity facts",
  "environmentWorld": "Describe only the locations actually used by this story",
  "charactersInvolved": ["Exact character names"],
  "keyThemes": ["theme"],
  "progression": [
    {"act":"Act 1","title":"...","summary":"...","characters":["..."],"keyActions":"...","dialogueSnippet":"..."}
  ],
  "dialogueHighlights": ["..."],
  "storyTone": "${settings.tone || 'Fun'}"
}

Create approximately ${targetScenes} meaningful story beats. Do not mention these instructions in the story.`;

  const result = await withRetry(() => model.generateContent(prompt));
  const parsed = JSON.parse(cleanJsonString(result.response.text()));
  const fullStory = String(parsed.fullStory || parsed.story || '').trim();
  if (!fullStory) throw new Error('AI story generation returned no story text.');

  return {
    storyMode: 'ai_create',
    storySource: 'ai_create',
    fullStory,
    sourceText: fullStory,
    refinedStory: fullStory,
    summary: parsed.summary || '',
    premise: parsed.premise || '',
    characterOverview: parsed.characterOverview || '',
    environmentWorld: parsed.environmentWorld || '',
    charactersInvolved: Array.isArray(parsed.charactersInvolved) ? parsed.charactersInvolved : [],
    keyThemes: Array.isArray(parsed.keyThemes) ? parsed.keyThemes : [],
    progression: Array.isArray(parsed.progression) ? parsed.progression : [],
    progressionBeats: Array.isArray(parsed.progression) ? parsed.progression : [],
    dialogueHighlights: Array.isArray(parsed.dialogueHighlights) ? parsed.dialogueHighlights : [],
    storyTone: parsed.storyTone || settings.tone || 'Fun',
    tone: parsed.storyTone || settings.tone,
    targetAudienceAnalysis: `${settings.audience || 'General'} audience in ${settings.language || 'English'}`,
  };
}
...
Respond ONLY with valid JSON (array of objects).`;
  ...
  const characters: CharacterProfile[] = list
    .filter((c: any) => {
      const name = String(c?.name || '').trim();
      if (!name) return false;
      // AI output must refer to an entity that actually exists in the active story.
      // This blocks stale/template characters from being reintroduced by the model.
      return activeStory.toLowerCase().includes(name.toLowerCase());
    })
    .map((c: any, index: number) => {
    const name = c.name || `Character ${index + 1}`;
    const storyAgeMatch = activeStory.match(new RegExp(`${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[^\n.]{0,120}?(\d{1,2})[- ]year[- ]old`, 'i'));
    const storyAge = storyAgeMatch?.[1] ? `${storyAgeMatch[1]} years old` : '';
    const charType = c.characterType || (index === 0 ? 'Protagonist' : 'Companion');
    const ageOrSpec = storyAge || c.age || c.species || c.ageOrSpecies || 'Not specified in story';
    ...
    return {
      ...
      age: ageOrSpec,
      ageOrSpecies: c.ageOrSpecies || storyAge || c.age || c.species || 'Not specified in story',
...
[
  {
    ...
    "aiVideoPrompt": "Complete photorealistic AI video prompt formatted with style, camera motion, lighting, and aspect ratio (${settings.aspectRatio || '9:16'})",
...
Respond ONLY with valid JSON.`;
  ...
  const scenes: SceneBreakdown[] = list.map((s: any, index: number) => {
    ...
    return {
      ...
      aiVideoPrompt: s.aiVideoPrompt || `Cinematic shot, ${s.location || 'narrative environment'}, ${cameraAngleMotion}, ${lightingMood}, photorealistic 8k, --ar ${settings.aspectRatio || '9:16'} ${settings.visualStyle}`,
...
Generate a JSON object matching this structure:
{
  ...
  "aiVideoPrompt": "Direct photorealistic AI video prompt for Sora/Runway (--ar ${settings.aspectRatio || '9:16'} ${settings.visualStyle})",
  ...
  "videoPrompts": [
    {
      ...
      "modelPrompts": {
        ...
        "runway": "Runway Gen-3 prompt: [Camera movement] + [Subject & Action] + [Environment & Lighting] --ar ${settings.aspectRatio || '9:16'}",
...
Respond ONLY with valid JSON.`;
  ...
  return list.map((item: any, idx: number) => {
    ...
    const modelPrompts: ModelSpecificPrompts = {
      ...
      runway: item.modelPrompts?.runway || item.finalPrompt || `${settings.visualStyle} scene ${sceneNum} --ar ${settings.aspectRatio || '9:16'}`,
    ...
    return {
      ...
      aspectRatio: item.aspectRatio || settings.aspectRatio || '9:16',
...
Generate a JSON object matching this exact structure:
{
  ...
  "modelPrompts": {
    ...
    "runway": "Runway Gen-3 prompt with camera motion and --ar ${settings.aspectRatio || '9:16'}",
...
Respond ONLY with valid JSON.`;
  ...
  const modelPrompts: ModelSpecificPrompts = {
    ...
    runway: parsed.modelPrompts?.runway || parsed.finalPrompt || existingPrompt?.modelPrompts?.runway || `${settings.visualStyle} scene ${sceneNumber} --ar ${settings.aspectRatio || '9:16'}`,
  ...
  const regenerated: SceneVideoPrompt = {
    ...
    durationSeconds: durationSec,
    aspectRatio: parsed.aspectRatio || settings.aspectRatio || '9:16',
