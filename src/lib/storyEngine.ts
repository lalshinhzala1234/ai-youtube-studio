...
export function extractAllAssetsUniversal(
  ...
  for (const [, entity] of discoveredEntities) {
    ...
    const age = entity.ageHint || 'Not specified in story';
    const ageCategory = entity.ageHint?.includes('Child') || /\b(?:[0-9]|1[0-7])(?:-year-old| years? old)?\b/i.test(entity.ageHint || '') ? 'Child' : entity.ageHint ? 'Young Adult' : 'Unspecified';
    ...
    const voiceLockId = `VOICE_${name.toUpperCase().replace(/[^A-Z0-9]+/g, '_')}_${gender === 'Girl' || gender === 'Female' ? 'GIRL' : gender === 'Boy' || gender === 'Male' ? 'BOY' : 'NEUTRAL'}_${ageCategory === 'Child' ? 'CHILD' : ageCategory === 'Young Adult' ? 'YOUNG' : 'UNSPECIFIED'}`;
    const voiceDescription = gender === 'Girl' || gender === 'Female'
      ? `Sweet, expressive ${ageCategory === 'Child' ? 'child' : ageCategory === 'Young Adult' ? 'young adult' : 'age-unspecified'} voice, enthusiastic ${lang} tone`
      : gender === 'Boy' || gender === 'Male'
      ? `${ageCategory === 'Child' ? 'curious child' : ageCategory === 'Young Adult' ? 'articulate young adult' : 'age-unspecified'} voice, energetic cheerful ${lang} tone`
      : `Natural age-appropriate voice, clear ${lang} tone`;
  ...
  // -------------------------------------------------------------
  const storyCombined = storyNarrative.toLowerCase();
  ...
  // 6. Kite / Rang-birangi Jadui Patang
  if (/patang|kite|kites|rang[- ]?birangi patang|colorful kite|magic(?:al)? kite|jadui patang/i.test(storyCombined)) {
    props.push({
      id: 'PROP_001_MAGIC_KITE',
      displayName: 'Rang-Birangi Jadui Patang (Colorful Magical Kite)',
      type: 'special_object',
      description: `A bright multicolored magical kite with a long flowing ribbon tail and subtle sparkling magical trails in ${style}.`,
      appearance: `Cheerful diamond-shaped kite made from vivid red, yellow, blue, green and pink panels, a wooden cross frame, long flowing ribbon tail and tiny luminous sparkles in ${style}.`,
      shape: 'Diamond-shaped traditional kite',
      materials: 'Lightweight paper fabric, thin wooden frame, colorful cotton string and ribbon tail',
      colors: 'Red, Yellow, Blue, Green, Pink, with warm golden sparkles',
      designDetails: 'Distinct five-color panel pattern, small decorative tassels and a long multicolored ribbon tail',
      scale: 'Child-sized kite visible clearly in medium and wide shots',
      usage: 'Flies above Chiku and Meera and drives the story action',
      style,
      lockedAttributes: ['Five-color diamond panel pattern', 'Long multicolored ribbon tail', 'Small golden magical sparkles'],
      lockedDesignAttributes: 'Exact five-color diamond kite with long ribbon tail and subtle magical sparkles',
      generationPrompt: `Prop reference for a colorful magical kite, five-color diamond panels, long flowing multicolored ribbon tail, subtle golden sparkles, ${style}, 8k --ar ${aspect}`,
      usageScenes: [1, 2, 3, 4, 5, 6],
      status: 'REFERENCE_READY',
      referenceImageStatus: 'READY',
    });
  }

  // No generic prop fallback: an empty prop registry is valid when the story has no concrete prop.
  ...
  const hasMeadow = /maidan|meadow|grassland|field|prairie|open\s+field/i.test(storyCombined);

  // 0. Open Meadow / Pahaadi Maidan
  if (hasMeadow) {
    environments.push({
      id: 'ENV_001_OPEN_MEADOW',
      displayName: 'Open Green Meadow / Pahaadi Maidan',
      type: 'environment',
      description: `Bright open green meadow with rolling grassy hills, wide blue sky, soft clouds and colorful wildflowers in ${style}.`,
      appearance: `Expansive cheerful hillside meadow with fresh green grass, gentle rolling terrain, scattered wildflowers, distant low hills and a clear blue sky with soft white clouds in ${style}.`,
      layout: 'Open rolling meadow with a broad unobstructed sky and distant hills',
      majorVisualLandmarks: 'Rolling grassy hill, wildflower patches, distant ridge line and open sky',
      lighting: 'Bright natural daylight with soft warm sunlight',
      timeOfDay: 'Daytime',
      colorPalette: ['Fresh Meadow Green', 'Sky Blue', 'Cloud White', 'Wildflower Pink', 'Sunlit Yellow'],
      groundOrBackgroundDetails: 'Soft grass, tiny flowers, gentle breeze and distant green hills',
      style,
      lockedAttributes: ['Open grassy meadow', 'Distant rolling hills', 'Wide blue sky with soft clouds'],
      generationPrompt: `Environment master concept for bright open green hilly meadow, rolling grass, wildflowers, distant hills, wide blue sky with soft clouds, ${style}, 8k --ar ${aspect}`,
      usageScenes: [1, 2, 3, 4, 5, 6],
      status: 'REFERENCE_READY',
      referenceImageStatus: 'READY',
    });
  ...
  // Standard individual environments (can coexist with the primary meadow).
  ...
  // No generic environment fallback. Only story-derived locations are registered.
...
export function generateScenesUniversal(
  ...
  return sceneDurations.map((durSec, i) => {
    ...
    // Determine the environment only from registered assets and the current story beat.
    // Never switch environments merely because a scene number changed.
    const environmentMatchesBeat = (env: EnvironmentProfile, text: string): boolean => {
      const id = env.id.toLowerCase();
      if (id.includes('meadow')) return /maidan|meadow|grass|field|prairie|open\s+field|hills?/i.test(text);
      if (id.includes('mountain')) return /pahad|pahadi|parvat|mountain|hill/i.test(text);
      if (id.includes('cave')) return /gufa|cave|cavern|surang|inside|interior|chamber/i.test(text);
      if (id.includes('forest') || id.includes('jungle')) return /jungle|van|forest|woods/i.test(text);
      if (id.includes('river')) return /nadi|river|riverbank|kinara|kinare|yamuna|ganga|lake|jheel/i.test(text);
      if (id.includes('path')) return /raasta|raaste|trail|path|marg/i.test(text);
      if (id.includes('swing')) return /jhoola|jhula|swing/i.test(text);
      return text.includes((env.displayName || '').split(/[\/()]/)[0].trim().toLowerCase());
    };

    let envObj = registeredEnvs.find((env) => environmentMatchesBeat(env, beatTextLower));
    if (!envObj && previousEnvId) {
      envObj = registeredEnvs.find((env) => env.id === previousEnvId);
    }
    if (!envObj) envObj = registeredEnvs[0];
    const envId = envObj?.id || 'NONE';
    ...
    // Determine props strictly from registered assets and the narrative beat.
    // Props can continue into later scenes, but scene indexes never create them.
    ...
    if (registeredProps.length > 0) {
      registeredProps.forEach((p) => {
        ...
        let matches = false;
        if (id.includes('kite') || name.includes('patang') || name.includes('kite')) matches = /patang|kite/i.test(beatTextLower);
        else if (id.includes('jhoola') || id.includes('jhula') || id.includes('swing') || name.includes('jhoola') || name.includes('jhula') || name.includes('swing')) matches = /jhoola|jhula|swing|jhul/i.test(beatTextLower);
        else if (id.includes('compass') || name.includes('compass')) matches = /compass|diksikshak|jadui compass/i.test(beatTextLower);
        else if (id.includes('map') || id.includes('naksha') || name.includes('naksha') || name.includes('map')) matches = /naksha|map|parchment|khazana|raasta dhoond/i.test(beatTextLower);
        else if (id.includes('torch') || id.includes('mashal') || name.includes('torch') || name.includes('mashal')) matches = /torch|mashal|flashlight|jalati|roshni/i.test(beatTextLower);
        else if (id.includes('bansuri') || name.includes('bansuri') || name.includes('flute')) matches = /bansuri|flute|murali/i.test(beatTextLower);
        else if (id.includes('hala') || name.includes('hala') || name.includes('plough')) matches = /hala|plough/i.test(beatTextLower);
        else matches = beatTextLower.includes(name.split(/[\/()]/)[0].trim());

        if (matches || (!isFirst && previousSceneProps.includes(p.id))) {
