...
export function extractAllAssetsUniversal(
  ...
  // 1. Compound Environment: Jungle Hill Cave Entrance
  if (hasCave && hasJungleOrHill) {
    environments.push({
...
export function generateScenesUniversal(
  ...
  return sceneDurations.map((durSec, i) => {
    ...
    if (isFirst) {
      startingAction = `${charNamesJoined || 'The protagonist'} stands on the jungle hillside trail at the mouth of ${envDisplayName}.`;
      finalAction = `${charNamesJoined || 'The protagonist'} gazes at the ancient cave entrance in awe.`;
    ...
    // Lyric lines (ONLY for English songs/rhymes, NEVER for story videos)
    ...
    // Spoken dialogue / narration in authentic Devanagari Hindi or English
    let dialogue = 'NONE';
    if (!ctx.isNoSpoken) {
      ...
      if (quoteMatch) {
        ...
        const speaker = sceneCharIds[0]
          ? registeredChars.find((c) => c.id === sceneCharIds[0])?.name || sceneCharIds[0]
          : 'Narrator';
        dialogue = `Speaker: ${sceneCharIds[0] || speaker} (${speaker})\nDialogue: "${quote}"`;
      } else if (isHindiOrHinglish) {
        // Resolve registered character profiles directly from the active story registry
        const meeraChar = registeredChars.find((c) => c.name.toLowerCase() === 'meera');
        const chikuChar = registeredChars.find((c) => c.name.toLowerCase() === 'chiku');

        if (i === 0 || (/pahunchte|pahunch|shuruaat|gufa ke paas|jungle ki pahadi/i.test(beatTextLower) && i === 0)) {
          // Meera speaks to Chiku upon arrival
          const speakerChar = meeraChar || registeredChars[0];
          dialogue = `Speaker: ${speakerChar.id} (${speakerChar.name})\nDialogue: "चीकू, देखो! हम जंगल की पहाड़ी पर बनी इस पुरानी गुफ़ा के पास पहुँच गए।"`;
        } else if (i === 1 || (/compass|nikalta|andar jaate/i.test(beatTextLower) && i === 1)) {
          // Chiku responds with his compass
          const speakerChar = chikuChar || registeredChars[0];
          dialogue = `Speaker: ${speakerChar.id} (${speakerChar.name})\nDialogue: "मीरा, मेरा कम्पास गुफ़ा के अंदर का रास्ता दिखा रहा है, चलो अंदर चलते हैं!"`;
        } else if (i === 2 || (/torch.*milti|khazane ka naksha/i.test(beatTextLower) && i === 2)) {
          // Meera discovers the map and torch
          const speakerChar = meeraChar || registeredChars[0];
          dialogue = `Speaker: ${speakerChar.id} (${speakerChar.name})\nDialogue: "चीकू, यहाँ देखो! एक पुराना ख़ज़ाने का नक्शा और एक टॉर्च रखी है!"`;
        } else if (i === 3 || /torch jalati|raasta dhoond|chhupa hua|gupt raasta/i.test(beatTextLower)) {
          // Chiku finds the hidden path
          const speakerChar = chikuChar || registeredChars[0];
          dialogue = `Speaker: ${speakerChar.id} (${speakerChar.name})\nDialogue: "मीरा, टॉर्च की रोशनी में देखो! नक्शे पर एक छुपा हुआ गुप्त रास्ता बना है, मिल गया रास्ता!"`;
        } else {
          const speakerChar = registeredChars[i % registeredChars.length] || registeredChars[0];
          dialogue = `Speaker: ${speakerChar.id} (${speakerChar.name})\nDialogue: "चलो आगे बढ़ते हैं और इस प्राचीन रहस्य को सुलझाते हैं!"`;
        }
      } else if (ctx.isNarratorOnly) {
        dialogue = `Speaker: Narrator\nNarration: "${beatText.slice(0, 140)}"`;
      } else {
        const speakerChar = registeredChars[0];
        dialogue = `Speaker: ${speakerChar ? `${speakerChar.id} (${speakerChar.name})` : 'Narrator'}\nDialogue: "${beatText.slice(0, 140)}"`;
      }
    }
...
export function generateVideoPromptsUniversal(
  ...
  return scenes.map((scene, idx) => {
    ...
    const dialogueText = scene.dialogue || scene.spokenDialogue || 'NO SPOKEN DIALOGUE';
    ...
    // Voice & Audio Lock
    let voiceAudioText = `Audio Mix: Balanced diegetic sound effects with orchestral score in ${settings.language}.`;
...
${consistencyText}
AUDIO & SFX:
${scene.soundEffects || 'Atmospheric Foley and melodic strings'}
ANIMATION:
${animationText}
CAMERA:
${cameraText}
