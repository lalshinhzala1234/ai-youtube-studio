...
export function createNewProjectPlaceholder(
  ...
  const project = generateCompleteProjectPackage(
    title,
    normalizedSettings,
    normalizedSettings.fullStory,
    normalizedSettings.characterInstructions,
    id
  );
...
export function generateCharactersForProject(
  ...
  return generateCharactersUniversal(
    title,
    settings,
    storyContent || settings.fullStory || settings.storyText || settings.refinedStory,
    characterInstructions || settings.characterInstructions
  );
}
...
export function generateScriptForProject(
  ...
  const resolvedChars =
    characters.length > 0
      ? characters
      : generateCharactersUniversal(title, settings, settings.fullStory, settings.characterInstructions);
  const resolvedScenes =
    scenes.length > 0
      ? scenes
      : generateScenesUniversal(title, settings, resolvedChars, settings.fullStory);
...
export function generateScenesForProject(
  ...
  return generateScenesUniversal(title, settings, resolvedChars, settings.fullStory);
}
...
export function generateVideoPromptsForProject(
  ...
  const resolvedScenes =
    scenes.length > 0
      ? scenes
      : generateScenesUniversal(title, settings, resolvedChars, settings.fullStory);
  return generateVideoPromptsUniversal(title, settings, resolvedScenes, resolvedChars, propsList, environmentsList);
}
