...
export const CharactersSection: React.FC<CharactersSectionProps> = ({
  ...
  const handleCopyAnchor = (char: CharacterProfile) => {
    navigator.clipboard.writeText(char.visualPromptAnchor || char.generationPrompt || char.appearance || '');
    setCopiedId(char.id);
    ...
    try {
      const response = await fetch('/api/generate/characters', {
        ...
        body: JSON.stringify({
          ...
          story: project.story?.fullStory || project.settings?.fullStory || project.settings?.storyText,
        }),
      });
      ...
      const updated: YouTubeProject = {
        ...
        assetRegistry: {
          ...
          characters: Object.fromEntries(newCharacters.map((c) => [c.id, c])),
        },
  ...
  const startEdit = (char: CharacterProfile) => {
    ...
    setName(char.name);
    setRole(char.role);
    setCharacterType(char.characterType || char.role || '');
  ...
  const saveEdit = (charId: string) => {
    ...
    const updated: YouTubeProject = {
      ...
      assetRegistry: {
        ...(project.assetRegistry || { characters: {}, props: {}, environments: {} }),
        characters: Object.fromEntries(updatedChars.map((c) => [c.id, c])),
      },
      updatedAt: new Date().toISOString(),
    };
    onUpdateProject(updated);
      ...
      <div className="p-5 rounded-2xl bg-[#161b22] border border-[#30363d] space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-emerald-400 text-xs font-semibold">
            ...
            <span>Locked Character Consistency Registry</span>
          </div>
        ...
        <p className="text-xs text-gray-300 leading-relaxed">
          These character profiles act as immutable reference visual blueprints for all generated scenes and video diffusion prompts. Reusing the locked visual prompt anchors guarantees facial, clothing, and aesthetic consistency across all video generation tools.
        </p>
        ...
        <div className="grid grid-cols-3 gap-3 pt-3 border-t border-[#30363d]">
          <div className="bg-[#0d1117] border border-[#21262d] rounded-lg p-3 text-center">
            <div className="text-xs text-gray-400 font-medium">TOTAL CHARACTERS REQUIRED</div>
            <div className="text-xl font-black text-white mt-0.5">{characters.length}</div>
          </div>
          <div className="bg-[#0d1117] border border-[#21262d] rounded-lg p-3 text-center">
            <div className="text-xs text-emerald-400 font-medium">REFERENCES READY</div>
            <div className="text-xl font-black text-emerald-400 mt-0.5">
              {characters.filter((c) => c.status === 'REFERENCE_READY' || c.referenceImageStatus === 'READY').length || characters.length}
            </div>
          </div>
          <div className="bg-[#0d1117] border border-[#21262d] rounded-lg p-3 text-center">
            ...
            <div className="text-xl font-black text-gray-300 mt-0.5">
              {Math.max(0, characters.length - (characters.filter((c) => c.status === 'REFERENCE_READY' || c.referenceImageStatus === 'READY').length || characters.length))}
            </div>
          </div>
        </div>
      </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-mono text-xs font-bold text-purple-400 bg-purple-950/40 border border-purple-800/40 px-2 py-0.5 rounded">
                          {char.id}
                        </span>
                ...
                {isEditing ? (
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                      ...
                      <div>
                        <label className="text-[10px] uppercase font-bold text-blue-400 block mb-1">Character Type</label>
                        <input
                          type="text"
                          value={characterType}
                          onChange={(e) => setCharacterType(e.target.value)}
                          placeholder="e.g. Human Boy, Robot, Animal"
