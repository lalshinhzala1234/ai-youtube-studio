                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 p-3 rounded-xl bg-[#0d1117] border border-[#21262d] text-xs">
                      <div className="flex items-center gap-2 text-gray-200">
                        ...
                        <span className="font-semibold">{scene.location || scene.environment}</span>
                        {scene.timeOfDay && (
                      ...
                      <div className="flex items-center gap-3 text-gray-300 flex-wrap">
                        ...
                        {scene.props && scene.props.length > 0 && (
                          <div className="flex items-center gap-1.5 text-amber-300">
                            ...
                            <span className="text-[11px] font-mono bg-amber-950/40 border border-amber-800/40 px-1.5 py-0.5 rounded">
                    ...
                    {/* Asset Dependencies & Continuity Tracking */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                      {/* Asset Dependencies Box */}
                      <div className="p-3 rounded-xl bg-[#090c10] border border-[#21262d] text-xs space-y-1.5">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-bold uppercase text-cyan-400 tracking-wider flex items-center gap-1">
                            <Layers className="w-3 h-3" /> Scene Asset Dependencies
                          </span>
                          <span className="text-[10px] text-emerald-400 font-semibold">Verified Registry</span>
                        </div>
                        <div className="space-y-1 text-[11px] text-gray-300">
                          <div>
                            <span className="text-gray-500">Characters: </span>
                            <span className="text-purple-300 font-mono">
                              {scene.assetDependencies?.characters.join(', ') || scene.characters?.join(', ') || 'None'}
                            </span>
                          </div>
                          <div>
                            <span className="text-gray-500">Environment: </span>
                            <span className="text-emerald-300 font-mono">
                              {scene.assetDependencies?.environment || scene.environment || scene.location}
                            </span>
                          </div>
                          <div>
                            <span className="text-gray-500">Props: </span>
                            <span className="text-amber-300 font-mono">
                              {scene.assetDependencies?.props.length ? scene.assetDependencies.props.join(', ') : 'None in shot'}
                            </span>
                          </div>
                        </div>
                      </div>

                      {/* Scene Continuity Info Box */}
                      <div className="p-3 rounded-xl bg-[#090c10] border border-[#21262d] text-xs space-y-1.5">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-bold uppercase text-pink-400 tracking-wider flex items-center gap-1">
                            <Zap className="w-3 h-3" /> Scene-to-Scene Continuity
                          </span>
                          <span className="text-[10px] text-amber-400 font-semibold">No Sudden Spawning</span>
                        </div>
                        <p className="text-[11px] text-gray-300 leading-relaxed">
                          {scene.continuity?.currentStartingAction || scene.continuityNote || `Continuous flow from Scene ${scene.sceneNumber - 1}`}
                        </p>
                      </div>
                    </div>
