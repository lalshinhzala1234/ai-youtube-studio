...
import { FREE_ACCESS_MODE, PLANS } from '@/lib/plans';
import {
  Sparkles,
  Zap,
  FileText,
    ...
    <div className="min-h-screen bg-[#090c10] text-[#f0f6fc] flex flex-col selection:bg-red-500 selection:text-white">
      ...
      <section id="pricing" className="py-20 px-6 bg-[#0d1117]/60 border-t border-[#21262d]">
        <div className="max-w-6xl mx-auto space-y-10">
          ...
          {/* Prominent Free Access Banner */}
          {FREE_ACCESS_MODE && (
            <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-red-500/15 via-rose-500/15 to-amber-500/15 border border-red-500/30 text-center space-y-1.5 shadow-lg shadow-red-500/5">
              <div className="flex items-center justify-center gap-2 text-sm sm:text-base font-bold text-white">
                <Gift className="w-5 h-5 text-red-400 animate-pulse" />
                <span>Creator Pro and Production Studio are FREE and fully unlocked right now.</span>
              </div>
              <p className="text-xs text-amber-300/90 font-medium">
                No payment required • No credit card required
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="p-8 rounded-2xl bg-[#161b22] border border-[#30363d] flex flex-col justify-between space-y-6">
              <div className="space-y-4">
                ...
                <p className="text-xs text-gray-400">
                  Perfect for exploring and creating your first complete video packages.
                </p>
                <ul className="space-y-2 text-xs text-gray-300">
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> 3 Full Video Packages / mo
                  </li>
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> Retention Hook Engine
                  </li>
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> Basic Scene Prompts
                  </li>
                </ul>
              </div>
              <button
                onClick={onGetStarted}
                className="w-full py-2.5 rounded-xl bg-[#21262d] hover:bg-[#30363d] text-white text-xs font-semibold transition-colors cursor-pointer"
              >
                Get Started Free
              </button>
            </div>

            {/* Creator Pro ($29 crossed out -> FREE NOW) */}
            <div className="p-8 rounded-2xl bg-[#161b22] border-2 border-red-500 flex flex-col justify-between space-y-6 relative shadow-xl shadow-red-500/10">
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full bg-red-600 text-[10px] font-bold uppercase tracking-wider text-white flex items-center gap-1 shadow-md">
                <Sparkles className="w-3 h-3" />
                <span>Temporary Free Access</span>
              </div>
              <div className="space-y-4">
                ...
                {FREE_ACCESS_MODE ? (
                  <div className="space-y-1">
                    ...
                    <div className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-red-400 via-rose-400 to-amber-300">
                      FREE NOW
                    </div>
                  </div>
                ) : (
                  <div className="text-3xl font-extrabold text-white">
                    $29 <span className="text-xs font-normal text-gray-400">/ month</span>
                  </div>
                )}

                <p className="text-xs text-gray-400">
                  For active YouTubers publishing weekly high-retention content.
                </p>
                <ul className="space-y-2 text-xs text-gray-300">
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> Unlimited Video Packages
                  </li>
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> Runway, Luma & Veo Prompts
                  </li>
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> High-CTR Thumbnail Studio
                  </li>
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> Full 9:16 Shorts Suite
                  </li>
                </ul>
              </div>
              <button
                onClick={onGetStarted}
                className="w-full py-2.5 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white text-xs font-bold transition-all shadow-md shadow-red-600/30 cursor-pointer flex items-center justify-center gap-1.5"
              >
                <span>{FREE_ACCESS_MODE ? 'Start Creator Pro — FREE' : 'Start Creator Pro'}</span>
                <ArrowRight className="w-3.5 h-3.5" />
              </button>
            </div>

            {/* Production Studio ($99 crossed out -> FREE NOW) */}
            <div className="p-8 rounded-2xl bg-[#161b22] border-2 border-purple-500/60 hover:border-purple-500 flex flex-col justify-between space-y-6 relative shadow-xl shadow-purple-500/5">
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full bg-purple-600 text-[10px] font-bold uppercase tracking-wider text-white flex items-center gap-1 shadow-md">
                <Sparkles className="w-3 h-3" />
                <span>Temporary Free Access</span>
              </div>
              <div className="space-y-4">
                ...
                {FREE_ACCESS_MODE ? (
                  <div className="space-y-1">
                    ...
                    <div className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-pink-400 to-rose-300">
                      FREE NOW
                    </div>
                  </div>
                ) : (
                  <div className="text-3xl font-extrabold text-white">
                    $99 <span className="text-xs font-normal text-gray-400">/ month</span>
                  </div>
                )}

                <p className="text-xs text-gray-400">
                  For multi-channel media networks and full video production teams.
                </p>
                <ul className="space-y-2 text-xs text-gray-300">
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> Multi-seat Team Collaboration
                  </li>
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> Character Consistency Library
                  </li>
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> CSV & Batch Export Pipelines
                  </li>
                </ul>
              </div>
              <button
                onClick={onGetStarted}
                className="w-full py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-xs font-bold transition-all shadow-md shadow-purple-600/30 cursor-pointer flex items-center justify-center gap-1.5"
              >
                <span>{FREE_ACCESS_MODE ? 'Unlock Production Studio — FREE' : 'Unlock Production Studio'}</span>
                <ArrowRight className="w-3.5 h-3.5" />
              </button>
            </div>
          </div>
