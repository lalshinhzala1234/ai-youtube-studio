...
export const NewProjectView: React.FC<NewProjectViewProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data?.success || !data?.story?.fullStory) {
        throw new Error(data?.error || 'AI story generation failed. No incomplete project was created.');
      }
      ...
      clearInterval(ticker);
      setGenerationError(err?.message || 'Story generation failed. No incomplete project was created.');
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#090c10] text-[#f0f6fc] p-6 lg:p-10">
      {generationError && (
        <div className="max-w-4xl mx-auto mb-5 p-4 rounded-xl bg-red-950/40 border border-red-500/40 text-sm text-red-200">
          {generationError}
        </div>
      )}
